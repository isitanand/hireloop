# Using jobhunt as a standalone CLI

Everything below describes the original command-line tool
(`python -m jobhunt ...`), which still works exactly as it always did — the
web app in `backend/` and `frontend/` is a full-stack layer built *on top
of* this package, not a replacement for it. If you just want a personal,
single-user tool with no web UI, no accounts, and no database, this is for
you.

> **New to Python?** Read **[SETUP.md](../SETUP.md)** instead — it's a
> 13-step guide that assumes you have nothing installed. This page assumes
...
funnel: 12 scanned -> 5 passed filters -> 5 new -> 2 in digest
```
...
tests/           63 tests, no network, no key
```
...
This is also the id the web app's shared job cache keys on (see
...
- the two bugs that cost an evening each: Lever's epoch-ms timestamps
